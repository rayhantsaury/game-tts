...
import useOutsideClick from "../../hooks/useOutsideClik";
import CellView from "./CellView";

function CellContainer({ cellOrientation, col, data, hasInput, hasNumber, number, onChange, onClick, onClickOutside, row, ...otherProps }) {
  ...
  function onKeyUp(e) {
    ...
    if (e.key === "Delete" || e.key === "Backspace") {
      const prevId = cellOrientation === "across" ? `${+dataset.col - 1},${dataset.row}` : `${dataset.col},${+dataset.row - 1}`;
      document.getElementById(prevId)?.select();
    } else if (e.key !== "") {
      const nextId = cellOrientation === "across" ? `${+dataset.col + 1},${dataset.row}` : `${dataset.col},${+dataset.row + 1}`;
