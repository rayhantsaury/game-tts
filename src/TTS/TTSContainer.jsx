...
import { puzzleData } from "../constans/puzzeData";
...
function calcCoords(list) {
  const entries = list.reduce((index, data) => {
    ...
    for (let j = 0; j < words.length; j += 1) {
      coords.push(j);
      const coord = data.orientation === "across" ? `${(data.startCol += 1) - 1},${data.startRow}` : `${data.startCol},${(data.startRow += 1) - 1}`;
      coords[j] = {
        coord,
        length: words.length,
        word: words[j],
  ...
  const cols = Object.keys(entries)
    .map((idx) => entries[idx].map((entry) => entry.coord.split(",")[0]))
    .flat(2);
  const rows = Object.keys(entries)
    .map((idx) => entries[idx].map((entry) => entry.coord.split(",")[1]))
    .flat(2);
...
function TTSContainer() {
  ...
  const convertedEntries = Object.keys(entries)
